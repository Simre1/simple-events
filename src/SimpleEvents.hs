...
newtype Event a =
  Event {registerHandler :: EventHandler a -> EventSetup (EventSetup ())}


-- | An 'EventHandler' describes how to deal with incoming events.
newtype EventHandler a =
  EventHandler {handleEvent :: a -> IO (EventHandler a)}

-- | An 'EventTrigger' triggers its corresponding event.
newtype EventTrigger a =
  EventTrigger {triggerEvent :: a -> IO ()}
...
newtype Behavior a =
  Behavior {behaviorValue :: EventSetup a}
  deriving (Functor, Applicative, Monad, Semigroup, Monoid)

-- | A 'Dynamic' is a combination of 'Event' and 'Behavior'.
data Dynamic a = Dynamic {behaviorFromDynamic :: Behavior a, eventFromDynamic :: Event a}

readBehavior :: MonadES m => Behavior a -> m a
readBehavior = liftES . behaviorValue

apply :: Behavior (a -> b) -> Event a -> Event b
apply behavior (Event reg) = Event $ \handlerB -> reg $ makeHandler handlerB
  where makeHandler handlerB = EventHandler $ \a -> do
          f <- readBehavior behavior
...
filterApply = ioFilterE . readBehavior
...
switchDynamics ds@(Dynamic bE evs) =
  let events = switchEvents $ eventFromDynamic <$> ds
      behavior = join $ behaviorFromDynamic <$> bE
...
instance Applicative Dynamic where
  ...
  (Dynamic acc1 (Event reg1)) <*> (Dynamic acc2 (Event reg2)) = Dynamic (acc1 <*> acc2) applicativeEvent
    where applicativeEvent = Event $ \f -> do
            unReg1 <- reg1 $ makeHandler (\ab -> (\a -> ab a) <$> readBehavior acc2) f
            unReg2 <- reg2 $ makeHandler (\a -> (\ab -> ab a) <$> readBehavior acc1) f
            pure (unReg1 *> unReg2)
            where
